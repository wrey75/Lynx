...
final class Acl {
	...
	static void parse(String str) {
		...
		while (i < b.length) {
				...
				if ((e - s) == 0) {
					...
					try {
						InetAddress[] addr = InetAddress
								.getAllByName(new String(b, s, e - s));
						byte[] bb;
			...
			if ((e - s) == 0) {
				...
				try {
					InetAddress[] addr = InetAddress.getAllByName(new String(b,
							s, e - s));
					byte[] bb;
					for (int j = 0; j < addr.length; j++) {
						bb = addr[j].getAddress();
						add(((bb[0] & 0xff) << 24) | ((bb[1] & 0xff) << 16)
								| ((bb[2] & 0xff) << 8) | ((bb[3] & 0xff)));
					}
				} catch (Exception ee) {
				}
	...
	static boolean check(InetAddress inet) {
		...
		int address = (((bb[0] & 0xff) << 24) | ((bb[1] & 0xff) << 16)
				| ((bb[2] & 0xff) << 8) | (bb[3] & 0xff));
		int i = 0;
		while (true) {
			if (hosts.length <= i || hosts[i] == 0)
				break;
